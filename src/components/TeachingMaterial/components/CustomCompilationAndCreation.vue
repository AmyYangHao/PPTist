...
const fetchTableData = async () => {
  const resp = await apiCommon.getCompilationAndCreationList({
...
const onInsert = (scope: API.CustomList) => {
  // chuang 互动编创
  // musicScore 自由编创
  emits("insert", `?q=custom_compose_music&id=${scope.id}`);
};
...
const updateHandle = () => {
  if (!editData.name?.trim()) return message.warning("请输入标题");

  apiCommon
    .updateCompilationAndCreation({ id: editData.id!, title: editData.name! })
    .then(() => {
...
const removeHandle = (scope: API.CustomList) => {
  apiCommon.deleteCompilationAndCreation(scope.id).then(() => {
