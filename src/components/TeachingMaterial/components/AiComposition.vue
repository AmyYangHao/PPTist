...
interface StateProps {
  ...
  tableData: API.PoetryListItemProps[];
}
...
const fetchTableData = async () => {
  const resp = await apiCommon.getPoetryList({
...
const onInsert = (scope: API.PoetryListItemProps) => {
  // -- 存储选中的数据，用于改造数据结构通过iframe发送给客户端存储展示
  // -- 在CanvasTool/index.vue页面插入iframe的代码块中，处理数据格式
  store.selectPoetInfo = scope;
  emits("insert", `?q=ai_composition&id=${scope.id}&source=ppt`);
};
...
<template>
  <el-form :inline="true" :model="state.formData" class="demo-form-inline">
    ...
    <el-form-item>
      <el-input v-model="state.formData.keyword" style="width: 240px" clearable placeholder="请输入诗歌名称" />
    </el-form-item>
    <el-form-item>
  ...
  <el-table :data="state.tableData" style="width: 100%" highlight-current-row empty-text="暂无数据">
    ...
    <el-table-column prop="name" label="诗歌名称" />
